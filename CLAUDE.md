...
Convert Excel (.xlsx) files to JSONL with robust merged-cell handling.
...
src/xl2jsonl/
  ...
  loader.py      - Dual-engine Excel reader (calamine + openpyxl) with merge resolution
...
1. **Loader** (`loader.py`): Reads Excel via calamine (fast, Rust-backed). In `auto` mode, probes each sheet for merged cells via openpyxl. Sheets with merges are re-read through openpyxl with full merge resolution (unmerge + fill).
